...
(function () {
  ...
  async function renderProgrammes(root, state) {
    ...
    try {
      ...
      if (items.length === 0) {
        resultsEl.innerHTML = '<div class="empty-state"><h3>No programmes match your filters.</h3><p>Try loosening the region or tier selection.</p></div>';
      } else {
  ...
  async function renderJobs(root, state) {
    ...
    try {
      ...
      if (items.length === 0) {
        resultsEl.innerHTML = '<div class="empty-state"><h3>No openings match your filters.</h3><p>Try changing the role or clearing the search text.</p></div>';
      } else {
